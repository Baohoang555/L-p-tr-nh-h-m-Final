...
updateProfile :: PlayerProfile -> LocalGameResult -> GameStats -> PlayerProfile
updateProfile profile result stats = 
  let profile1 = profile 
      ...
      profile3 = profile2 -- (Phần logic này đã được thêm lại)
...
updateRatings :: Int -> Int -> LocalGameResult -> (Int, Int)
updateRatings rating1 rating2 result = 
  let k = 32.0
      ...
      (actual1, actual2) = case result of
        LocalWin p -> if playerWon p then (1.0, 0.0) else (0.0, 1.0)
        LocalDraw -> (0.5, 0.5)
...
testStatistics = do
  ...
  let alice1 = updateProfile alice (LocalWin Red) stats1 -- Đã sửa
  
  let stats2 = GameStats "2" "Alice" "Charlie" (Just "Alice") 25 (Just 90) "2025-01-02"
  let alice2 = updateProfile alice1 (LocalWin Red) stats2 -- Đã sửa
