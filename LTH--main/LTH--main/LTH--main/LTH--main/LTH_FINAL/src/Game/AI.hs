...
showBoardPretty board = unlines $
  ...
  [ show row ++ "|" ++ unwords [showCellPretty (getCell board (col, row)) | col <- [0..6]]
    | row <- [5,4..0]
  ] ++
  [ "  " ++ replicate 14 '-' ]

showCellPretty :: Cell -> String
showCellPretty Empty = "·"
showCellPretty (Occupied Red) = "🔴"
showCellPretty (Occupied Black) = "⚫"
